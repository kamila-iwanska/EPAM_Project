...
function render() {
    ...
    const templateEl = document.getElementById('cart-template');
    let subTotal = 0;
    bodyEl.innerHTML = '';
    for (const cartItem of getAllCartItems()) {
        const product = getProductById(cartItem.productId);
        subTotal += product.price * cartItem.quantity;
        const newRowEl = templateEl.content.cloneNode(true);
        const newRowImageEl = newRowEl.querySelector('.cart-template-image');
