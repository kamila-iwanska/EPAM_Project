...
function initializeHomeView() {
  const productTemplateEl = document.getElementById("home-product-template") as HTMLTemplateElement;

  blocks.forEach(({ parentId, block }) => {
    const products = getProductsForBlock(block);
    
    products.forEach((product) => {
      const productEl = productTemplateEl.content.cloneNode(true) as HTMLElement;
      const productImageEl = productEl.querySelector('.home-product-image') as HTMLImageElement;
